...
public class Listeners implements Listener
{
    ...
    private void onDeath(final PlayerDeathEvent e) {
        if (e.getEntity().getKiller() instanceof Player) {
            ...
            if (this.plugin.participantes.contains(killer.getName()) && this.plugin.participantes.contains(e.getEntity().getName())) {
                ...
                killer.sendMessage(ChatColor.GOLD + "[MiniGladiador] " + ChatColor.YELLOW + "Voc\u00ea tem " + ChatColor.RED + (k + 1) + ChatColor.YELLOW + " kill!");
                Main.pl.getSqlite().addKillerPoint(killer.getName().toString());
            }
        }
        this.plugin.removePlayer(e.getEntity(), 1);
        this.plugin.checkMiniGladiadorEnd_core1();
    }
    ...
    private void onDamage(final EntityDamageByEntityEvent e) {
        if (this.plugin.getMiniGladiadorEtapa() != 0 && e.getEntity() instanceof Player && (e.getDamager() instanceof Player || e.getDamager() instanceof Projectile)) {
            ...
            if (this.plugin.participantes.contains(ent.getName()) && this.plugin.getMiniGladiadorEtapa() != 3) {
                e.setCancelled(true);
                if (dam != null) {
                    dam.sendMessage(ChatColor.RED + "PvP desativado no momento!");
                }
            }
            if (dam != null && this.plugin.getMiniGladiadorEtapa() == 3 && this.plugin.participantes.contains(ent.getName()) && this.plugin.participantes.contains(dam.getName())) {
    ...
    private void onDamageP(final PotionSplashEvent e) {
        for (final Entity ent2 : e.getAffectedEntities()) {
            if (ent2 instanceof Player && this.plugin.getMiniGladiadorEtapa() != 0) {
                final Player ent3 = (Player)ent2;
                Player dam = null;
                if (e.getPotion().getShooter() instanceof Player) {
                    dam = (Player)e.getEntity().getShooter();
                }
                if (this.plugin.participantes.contains(ent3.getName()) && this.plugin.getMiniGladiadorEtapa() != 3) {
                    e.setCancelled(true);
                    if (dam != null) {
                        dam.sendMessage(ChatColor.RED + "PvP desativado no momento!");
                    }
                }
                if (dam == null || this.plugin.getMiniGladiadorEtapa() != 3 || !this.plugin.participantes.contains(ent3.getName())) {
                    continue;
                }
    ...
    private void onPCmd(final PlayerCommandPreprocessEvent e) {
        if (!this.plugin.specs.isEmpty() && this.plugin.specs.contains(e.getPlayer().getName()) && e.getMessage().toLowerCase().startsWith("/") && !e.getMessage().startsWith("/camarote sair")) {
            e.setCancelled(true);
            e.getPlayer().sendMessage(new String[] { "§6[MiniGladiador] §cVoc\u00ea nao pode usar comandos no modo espectador!", "§6[MiniGladiador] §ePara sair clique na cama do seu invent\u00e1rio!" });
        }
        if (this.plugin.getMiniGladiadorEtapa() == 3 && e.getMessage().toLowerCase().startsWith("/clan") && !e.getMessage().toLowerCase().startsWith("/clan resign") && !e.getMessage().toLowerCase().startsWith("/clan kick") && !e.getMessage().toLowerCase().startsWith("/clan invite") && !e.getMessage().toLowerCase().startsWith("/clan trust") && !e.getMessage().toLowerCase().startsWith("/clan profile") && e.getMessage().toLowerCase().startsWith("/x9") && e.getMessage().toLowerCase().startsWith("/sgcraft:x9")) {
            e.getPlayer().sendMessage(ChatColor.RED + "Comando bloqueado no evento MiniGladiador!");
            e.setCancelled(true);
        }
        else if (this.plugin.getMiniGladiadorEtapa() != 0 && e.getMessage().toLowerCase().startsWith("/clan modtag")) {
            e.setCancelled(true);
        }
    }
