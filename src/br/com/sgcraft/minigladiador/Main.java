...
import br.com.sgcraft.minigladiador.chat.*;
import br.com.sgcraft.minigladiador.hooks.*;
...
public class Main extends JavaPlugin
{
    ...
    private int minigladiadorEtapa;
    ...
    public void onEnable() {
        Main.pl = this;
        this.getLogger().info("Ativando MiniGladiador (V" + this.getDescription().getVersion() + ") - Autor: Sardinhagamer_HD");
        ...
        this.getServer().getPluginCommand("minigladiador").setExecutor((CommandExecutor)new Comando(this));
        this.getServer().getPluginCommand("minigladiadores").setExecutor((CommandExecutor)new Comando(this));
        this.getServer().getPluginCommand("minicastelo").setExecutor((CommandExecutor)new Utils(this));
    ...
    public void onDisable() {
        this.getLogger().info("Desativando MiniGladiador - Autor: Sardinhagamer_HD");
    }
    ...
    protected void prepareMiniGladiador() {
        if (this.minigladiadorEtapa != 0) {
            return;
        }
        final String[] ent = this.getConfig().getString("Arena.Entrada").split(";");
        this.spawn = new Location(this.getServer().getWorld(ent[0]), Double.parseDouble(ent[1]), Double.parseDouble(ent[2]), Double.parseDouble(ent[3]), Float.parseFloat(ent[4]), Float.parseFloat(ent[5]));
    ...
    private void messagePrepare(final int vezes) {
        this.canStart = true;
        if (this.minigladiadorEtapa != 1) {
            return;
        }
        this.canStart = false;
        if (vezes == 0) {
            this.preparedMiniGladiador();
        }
        else {
        ...
        this.getServer().getScheduler().runTaskLater((Plugin)this, (Runnable)new Runnable() {
            ...
            public void run() {
                ...
                if (Main.this.minigladiadorEtapa != 1) {
                    return;
                }
    ...
    private void messageIniciando(final int vezes) {
        this.canStart = true;
        if (this.minigladiadorEtapa != 2) {
            return;
        }
        this.canStart = false;
        if (vezes == 0) {
            this.startMiniGladiador();
        }
        else {
            for (final String n : Mensagens.getIniciando()) {
                ...
                this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "title &e&lMINIGLADIADOR<nl>&bPrepare-se" );
            }
        ...
        this.getServer().getScheduler().runTaskLater((Plugin)this, (Runnable)new Runnable() {
            ...
            public void run() {
                Main.this.canStart = true;
                if (Main.this.minigladiadorEtapa != 2) {
                    return;
                }
                Main.this.canStart = false;
    ...
    protected void startMiniGladiador() {
        if (this.getClansParticipandoSize() < 2) {
            this.cancelMiniGladiador();
            for (final String n : Mensagens.getCancelar()) {
                this.getServer().broadcastMessage(n.replace("&", "§"));
                this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "title &e&lMINIGLADIADOR<nl>&cEvento Cancelado!" );
            }
            return;
        }
        this.canStart = true;
        this.minigladiadorEtapa = 3;
	...
	protected void checkMiniGladiadorEnd_core1() {
        final List<Clan> lista = this.getClansParticipando_core1();
        if (lista.size() == 1 && this.minigladiadorEtapa == 3) {
            this.minigladiadorEtapa = 4;
            ...
            for (final String n : Mensagens.getFinalizar()) {
            	this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "title &6&lMINIGLADIADOR<nl>&7Parabéns "+ vencedor.getColorTag() );
                if ((this.mito || !n.startsWith("(mito)")) && (!this.mito || !n.startsWith("(gladiador)"))) {
                    if (n.startsWith("(gladiador)")) {
                        ...
                        this.getServer().broadcastMessage(n.replace("&", "§").substring(6).replace("@clan", vencedor.getColorTag()).replace("@dinheiro", Double.toString(this.getConfig().getDouble("Premios.Dinheiro"))));
                    }
                    else if (!this.mito) {
            ...
            this.sendMessageMiniGladiador(Mensagens.getMensagem("Final1").replace("@tempo", Integer.toString(this.getConfig().getInt("Timers.Finalizando"))));
            this.getServer().dispatchCommand((CommandSender)this.getServer().getConsoleSender(), "clan globalff auto");
            this.getServer().getScheduler().runTaskLater((Plugin)this, (Runnable)new Runnable() {
                @Override
                public void run() {
    ...
    protected void finalizarGladiador() {
        this.sendMessageMiniGladiador(Mensagens.getMensagem("Final2"));
        this.cancelMiniGladiador();
    }
    ...
    protected void cancelMiniGladiador() {
        if (this.minigladiadorEtapa == 0) {
            return;
        }
        if (this.getMiniGladiadorEtapa() > 2) {
    ...
    public int getMiniGladiadorEtapa() {
        return this.minigladiadorEtapa;
    }
    ...
    protected void removePlayer(final Player p, final int motive) {
        ...
        else if (this.minigladiadorEtapa == 3) {
            this.checkMiniGladiadorEnd_core1();
        }
        p.teleport(this.saida);
        if (this.minigladiadorEtapa == 1) {
            this.totalParticipantes.remove(p.getName());
    ...
    protected void sendMessageMiniGladiador(final String msg) {
        for (final String n : this.participantes) {
